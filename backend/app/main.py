...
def create_category_for_user(
    ...
    db_category = crud.get_user_category_by_name(db, user_id=current_user.id, name=category.name)
    if db_category:
...
def create_transaction(
    ...
    if transaction.category_id:
        category = crud.get_user_category(db, category_id=transaction.category_id, user_id=current_user.id)
        if not category:
            raise HTTPException(status_code=404, detail="La categoría no existe o no pertenece al usuario")
...
def update_transaction(
    ...
    db_transaction = crud.get_user_transaction(db, user_id=current_user.id, transaction_id=transaction_id)
    if db_transaction is None:
        raise HTTPException(status_code=404, detail="Transacción no encontrada")

    if transaction_update.category_id:
        category = crud.get_user_category(db, category_id=transaction_update.category_id, user_id=current_user.id)
        if not category:
...
def get_dashboard_summary_by_date(
    ...
    summary = crud.get_summary_by_date_range(db, user_id=current_user.id, start=start_date, end=end_date)
    return summary