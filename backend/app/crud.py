...
def create_user(db: Session, user: schemas.UserCreate):
    try:
        ...
        pw_bytes_len = (
            ...
            else len(repr(pw).encode("utf-8"))
        )
        print(
            f"[DEBUG] create_user password type={pw_type} bytes_len={pw_bytes_len} repr={pw_repr}"
        )
...
def get_user_category_by_name(db: Session, user_id: int, name: str):
    return (
        db.query(models.Category)
        .filter(models.Category.name == name, models.Category.owner_id == user_id)
        .first()
    )
